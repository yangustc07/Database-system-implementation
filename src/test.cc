...
void q2 (){
	...
	//s.Apply(final, relName, 2);
	s.Estimate(final, relName, 2);//

	//cnf = " (c_nationkey = n_nationkey)";
	cnf = "(c_custkey = o_custkey) AND (c_nationkey = n_nationkey)";//
	yy_scan_string(cnf);
	yyparse();
...
void q3 (){
	...
	double result = s.Estimate(final, set3, 2);//
	if(fabs(result-60000000.0)>0.1)
		cout<<"error in estimating Q3\n";

	s.Apply(final, set3, 2);//
...
//	cnf ="(s.s_suppkey = ps.ps_suppkey)";
	cnf ="(p.p_partkey=ps.ps_partkey) AND (p.p_size = 2) AND (ps.ps_suppkey = s.s_suppkey)";//
	yy_scan_string(cnf);
	yyparse();
	//char * relName2[] = {"p", "ps", "s"};//
	s.Estimate(final, relName, 3);//
	s.Apply(final, relName, 3);//

	//char * relName3[] = {"p|ps|s", "n"};//
...
void q5 (){
	...
	cnf = " (o_orderkey = l_orderkey) ";
	yy_scan_string(cnf);
	yyparse();

	//char * relName1[] = {"customer|orders", "lineitem"};//
	double result = s.Estimate(final, relName, 2);//
...
void q6 (){
	...
	//char * relName1[] = {"partsupp|supplier", "nation"};//
...
void q9(){
	...
	//char * relName1[] = {"part|partsupp", "supplier"};//
	cnf = " (ps_suppkey = s_suppkey)";//
	yy_scan_string(cnf);
	yyparse();
...
void q10 (){
	...
	yy_scan_string(cnf);                                                                               	
	yyparse();	
	//char * relName2[] = {"customer|orders|lineitem", "nation"};//
	double result = s.Estimate(final, relName, 2);//
	if(fabs(result-2000405)>0.1)
